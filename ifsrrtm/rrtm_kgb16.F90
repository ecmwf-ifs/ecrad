...
INTEGER :: IG
REAL(KIND=JPRD) :: ZTMP_KAO_D(9,5,13,16)
REAL(KIND=JPRD) :: ZTMP_KBO_D(5,13:59,16)
...
IF( MYPROC==1 )THEN
  READ(NULRAD,ERR=1001) ZTMP_KAO_D,ZTMP_KBO_D
  CLOSE(NULRAD,ERR=1000)
  DO IG = 1, 16
    KAO_D(IG,:,:,:) = ZTMP_KAO_D(:,:,:,IG)
    KBO_D(IG,:,:) = ZTMP_KBO_D(:,:,IG)
  ENDDO
  KAO = REAL(KAO_D,JPRB)
  KBO = REAL(KBO_D,JPRB)
ENDIF
