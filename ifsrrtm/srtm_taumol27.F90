...
!CDIR UNROLL=NG27
        DO IG = 1 , NG27
          ...
          P_TAUG(IG,IPLON,I_LAY) = P_COLO3(IPLON,I_LAY) * &
           & (P_FAC00(IPLON,I_LAY) * ABSA(IG,IND0) + &
           & P_FAC10(IPLON,I_LAY) * ABSA(IG,IND0+1) + &
           & P_FAC01(IPLON,I_LAY) * ABSA(IG,IND1) + &
           & P_FAC11(IPLON,I_LAY) * ABSA(IG,IND1+1))  
          !     &          + TAURAY
...
!CDIR UNROLL=NG27
        DO IG = 1 , NG27
          ...
          !     &          + TAURAY
          !    SSA(LAY,IG) = TAURAY/TAUG(LAY,IG)
          IF (I_LAY == I_LAYSOLFR(IPLON)) P_SFLUXZEN(IG,IPLON) = SCALEKUR * SFLUXREFC(IG) 
          P_TAUR(IG,IPLON,I_LAY) = Z_TAURAY
        ENDDO
      ENDIF
    ENDIF
