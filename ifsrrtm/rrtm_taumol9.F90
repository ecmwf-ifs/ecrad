...
!-- DS_000515
        ...
        ZN2OM1 = KA_MN2O(IG,JMN2O,INDM(JLAY)) + Z_FMN2O * &
          &      (KA_MN2O(IG,JMN2O+1,INDM(JLAY)) - KA_MN2O(IG,JMN2O,INDM(JLAY)))
        ZN2OM2 = KA_MN2O(IG,JMN2O,INDM(JLAY)+1) + Z_FMN2O * &
          &      (KA_MN2O(IG,JMN2O+1,INDM(JLAY)+1) - KA_MN2O(IG,JMN2O,INDM(JLAY)+1))
   ...
   IF (Z_SPECPARM < 0.125_JPRB) THEN
        ...
        P_TAU(NGS8+IG,JLON,JLAY) = ZTAU_MAJOR + ZTAU_MAJOR1 &
         &       + ZTAUSELF + ZTAUFOR &
         &       + ZADJCOLN2O(JLON,JLAY)*ZABSN2O &
         & + P_TAUAERL(JLON,JLAY,9)  
        PFRAC(NGS8+IG,JLON,JLAY) = FRACREFA(IG,JPL) + Z_FPL *&
...
!CDIR UNROLL=NG9
      DO IG = 1, NG9
        ZABSN2O = KB_MN2O(IG,INDM(JLAY)) + PMINORFRAC(JLON,JLAY) * &
         &       (KB_MN2O(IG,INDM(JLAY)+1) - KB_MN2O(IG,INDM(JLAY)))

!-- JJM_000517
        P_TAU(NGS8+IG,JLON,JLAY) = P_COLCH4(JLON,JLAY) *&
         ...
         & + ZADJCOLN2O(JLON,JLAY)*ZABSN2O &
         & + P_TAUAERL(JLON,JLAY,9)  
        PFRAC(NGS8+IG,JLON,JLAY) = FRACREFB(IG)
      ENDDO
    ENDIF
  ENDDO
ENDDO
