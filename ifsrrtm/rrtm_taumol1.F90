...
!CDIR UNROLL=NG1
    DO IG = 1, NG1
!-- DS_000515  
           ZTAUSELF = P_SELFFAC(JLON,JLAY) * (SELFREF(IG,INDS(JLAY)) + &
     &           P_SELFFRAC(JLON,JLAY) * &
     &           (SELFREF(IG,INDS(JLAY)+1) - SELFREF(IG,INDS(JLAY)))) 

            ZTAUFOR =  P_FORFAC(JLON,JLAY) * (FORREF(IG,INDF(JLAY)) + &
     &           P_FORFRAC(JLON,JLAY) * (FORREF(IG,INDF(JLAY)+1) - &
     &           FORREF(IG,INDF(JLAY))))  

            ZTAUN2 = ZSCALEN2*(KA_MN2(IG,INDM(JLAY)) +   &
     &           P_MINORFRAC(JLON,JLAY) * &
     &           (KA_MN2(IG,INDM(JLAY)+1) - KA_MN2(IG,INDM(JLAY)))) 

            P_TAU(IG,JLON,JLAY) = ZCORRADJ * (P_COLH2O(JLON,JLAY) *  &
     &          (P_FAC00(JLON,JLAY) * ABSA(IG,IND0(JLAY)) + &
     &           P_FAC10(JLON,JLAY) * ABSA(IG,IND0(JLAY)+1) + &
     &           P_FAC01(JLON,JLAY) * ABSA(IG,IND1(JLAY)) +  &
     &           P_FAC11(JLON,JLAY) * ABSA(IG,IND1(JLAY)+1))  &
     &           + ZTAUSELF + ZTAUFOR &
     &           + ZTAUN2) + P_TAUAERL(JLON,JLAY,1)

            PFRAC(IG,JLON,JLAY) = FRACREFA(IG)
...
!-- JJM000517
     ...
     &           (FORREF(IG,INDF(JLAY)+1) - FORREF(IG,INDF(JLAY)))) 
            ...
            ZTAUN2 = ZSCALEN2*(KB_MN2(IG,INDM(JLAY)) +  &
     &           P_MINORFRAC(JLON,JLAY) * &
     &           (KB_MN2(IG,INDM(JLAY)+1) - KB_MN2(IG,INDM(JLAY))))
     ...
     &           + ZTAUFOR &
     &           + ZTAUN2)+ P_TAUAERL(JLON,JLAY,1)
      PFRAC(IG,JLON,JLAY) = FRACREFB(IG)
