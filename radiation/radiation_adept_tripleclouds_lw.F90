...
module radiation_adept_tripleclouds_lw

  use iso_c_binding

contains

  subroutine solver_adept_tripleclouds_lw(nlev,istartcol,iendcol, &
...
#include "tcrad.inc"
    ...
    if (lhook) call dr_hook('radiation_adept_tripleclouds_lw:solver_adept_tripleclouds_lw',0,hook_handle)
    ...
    do jcol = istartcol,iendcol
      ...
      if (config%do_lw_cloud_scattering) then
        ...
        call tcrad_flux_lw(config%n_g_lw, nlev, tc_config, &
    ...
    if (lhook) call dr_hook('radiation_adept_tripleclouds_lw:solver_adept_tripleclouds_lw',1,hook_handle)

  end subroutine solver_adept_tripleclouds_lw


  subroutine radiance_solver_adept_tripleclouds_lw(nlev,istartcol,iendcol, &
...
#include "tcrad.inc"
    ...
    do jcol = istartcol,iendcol
      ...
      if (config%do_lw_cloud_scattering) then
        ...
        call tcrad_radiance_lw(config%n_g_lw, nlev, tc_config, cos_sensor_zenith_angle(jcol), &
             &         emission(:,jcol), albedo(:,jcol), &
  ...
  subroutine radiance_solver_adept_tripleclouds_lw_ad(nlev,istartcol,iendcol, &
       &  config, thermodynamics, cloud, cos_sensor_zenith_angle, &
       &  od, ssa, g, od_cloud, ssa_cloud, g_cloud, planck_hl, &
       &  emission, albedo, flux)
...
#include "tcrad.inc"
    ...
    real(jprb), dimension(nlev) :: cloud_fraction_ad
    ...
    do jcol = istartcol,iendcol
      ...
      if (config%do_lw_cloud_scattering) then
        ...
        call tcrad_radiance_lw_ad(config%n_g_lw, nlev, tc_config, cos_sensor_zenith_angle(jcol), &
             &         emission(:,jcol), albedo(:,jcol), &
             &         planck_hl(:,:,jcol), cloud%fraction(jcol,:), cloud%fractional_std(jcol,:), &
             &         od(:,:,jcol), od_cloud_regrid, ssa_cloud_regrid, g_cloud_regrid, &
             &         cloud%overlap_param(jcol,:), &
             &         spectral_radiance, &
             &         spectral_radiance_ad, &
             &         emission_ad, albedo_ad, planck_hl_ad, cloud_fraction_ad, od_ad, od_cloud_ad, &
  ...
  end subroutine radiance_solver_adept_tripleclouds_lw_ad
  
end module radiation_adept_tripleclouds_lw
