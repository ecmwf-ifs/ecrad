! radiation_optimal_columns_sw.F90 - Optimal columns shortwave solver
!
...
!
! This flux solver uses the Optimal Columns algorithm to generate a
! small number of cloudy sub-columns whose top-of-atmosphere
! reflectance should be a good approximation to that computed using a
! more accurate solver. The heating-rate profile is unlikely to be
! accurate. The main purpose of this solver is to help test the
! Optimal Columns algorithm for application in a solar radiance model.
!
! Modifications
  
#include "ecrad_config.h"

module radiation_optimal_columns_sw
...
#include "radiation_delta_eddington.h"
  ...
  !---------------------------------------------------------------------
  ! Shortwave Optimal Columns solver, adapted from
  ! radiation_mcica_sw.F90 but using the Optimal Columns rather than
  ! the stochastic cloud generator.  
  subroutine solver_optimal_columns_sw(nlev,istartcol,iendcol, &
    ...
    if (lhook) call dr_hook('radiation_optimal_columns_sw:solver_optimal_columns_sw',0,hook_handle)

    if (.not. config%do_clear) then
      write(nulerr,'(a)') '*** Error: shortwave Optimal Columns requires clear-sky calculation to be performed'
    ...
    if (use_stochastic_columns_local) then
      ...
      ! Note: cloud properties are on bands, but these can be the same as g-points
    ...
    if (lhook) call dr_hook('radiation_optimal_columns_sw:solver_optimal_columns_sw',1,hook_handle)
    
  end subroutine solver_optimal_columns_sw

end module radiation_optimal_columns_sw
