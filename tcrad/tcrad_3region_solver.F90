...
#undef NUM_REGIONS
  ...
  !---------------------------------------------------------------------
  ! Compute the flux profile including the effects of scattering,
  ! either using the classic Tripleclouds solver alone, or using it to
  ! compute the source function for subsequent radiance calculations. 
  subroutine calc_flux(nspec, nlev, surf_emission, surf_albedo, planck_hl, &
    ...
    if (lhook) call dr_hook('tcrad_3region_solver:calc_flux',0,hook_handle)
    ...
    if (lhook) call dr_hook('tcrad_3region_solver:calc_flux',1,hook_handle)
 ...
 subroutine calc_no_scattering_flux(nspec, nlev, surf_emission, surf_albedo, planck_hl, &
    ...
    if (lhook) call dr_hook('tcrad_3region_solver:calc_no_scattering_flux',0,hook_handle)
    ...
    if (lhook) call dr_hook('tcrad_3region_solver:calc_no_scattering_flux',1,hook_handle)
