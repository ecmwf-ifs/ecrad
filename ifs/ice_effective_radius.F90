...
CASE(1,2)
  ...
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) PRIVATE(ZTEMPERATURE_C) IF(LLACC)
...
CASE(3)
  ...
  !$OMP TARGET ENTER DATA MAP(ALLOC:ZMIN_DIAMETER_UM) IF(LLACC)
  ...
  !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) PRIVATE(ZAIR_DENSITY_GM3, ZIWC_INCLOUD_GM3, &
  !$OMP& ZTEMPERATURE_C, ZAIWC, ZBIWC, ZDIAMETER_UM) IF(LLACC)
  ...
  !$ACC END DATA
  !$OMP TARGET EXIT DATA MAP(DELETE:ZMIN_DIAMETER_UM)
