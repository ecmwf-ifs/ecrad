...
PRIVATE
PUBLIC :: RADIATION_SCHEME_LAYER, RADIATION_SCHEME_LAYER_PARALLEL, RADIATION_SCHEME_LAYER_OPENACC
...
! Dummies for IFS config values
LOGICAL :: LSPPRAD, LRAYFM, LEPO3RA, LBITIDENTITY
LOGICAL, PARAMETER :: LDEBUG = .false.
...
REAL(KIND=JPHOOK) :: ZHOOK_HANDLE, ZHOOK_HANDLE_OFFLOAD, ZHOOK_HANDLE_COMPUTE, ZHOOK_HANDLE_PULLBACK

#include "radiation_scheme.intfb.h"

IF (LHOOK) CALL DR_HOOK('RADIATION_SCHEME_LAYER_OPENACC',0,ZHOOK_HANDLE)
...
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 3, F_iamu0)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 4, F_iemiss)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 5, F_its)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 6, F_islm)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 7, F_iccnl)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 8, F_iccno)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 11, F_igelam)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 12, F_igemu)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 15, F_iald)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 16, F_ialp)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 17, F_iti)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 18, F_ipr)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 20, F_iwv)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 21, F_iclc)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 22, F_ilwa)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 23, F_iiwa)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 24, F_iswa)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 25, F_irwa)
IF(lrayfm) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 28, F_ioz)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 31, F_iaprs)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 32, F_ihti)
IF(lbitidentity) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 33, F_ire_liq)
IF(lbitidentity) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 34, F_ire_ice)
IF(lbitidentity) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 35, F_ioverlap)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 36, F_iaero)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 37, F_ifrsod)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 38, F_ifrted)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 39, F_ifrsodc)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 40, F_ifrtedc)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 41, F_iemit)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 42, F_isudu)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 43, F_iuvdf)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 44, F_iparf)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 45, F_iparcf)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 46, F_itincf)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 47, F_ifdir)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 49, F_icdir)
IF(lapproxlwupdate) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 50, F_ilwderivative)
IF(lapproxswupdate) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 51, F_iswdirectband)
IF(lapproxswupdate) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 52, F_iswdiffuseband)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 53, F_ifrso)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 54, F_iswfc)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 55, F_ifrth)
IF(.true.) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 56, F_ilwfc)
IF(ldiagforcing) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 57, F_iaer)
IF(ldiagforcing) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 58, F_ioz)
IF(ldiagforcing) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 59, F_iico2)
IF(ldiagforcing) CALL GET_STACK_MEMBER(ZRGP_FIELDS%FIELD_WRAPPER, 60, F_iich4)
...
IF (LHOOK) CALL DR_HOOK('RADIATION_SCHEME_LAYER_OPENACC:OFFLOAD',0,ZHOOK_HANDLE_OFFLOAD)

CALL ZRGP_FIELDS%FIELD_WRAPPER%SYNC_DEVICE_RDWR()

IF(.true.) CALL F_iamu0%GET_DEVICE_DATA_RDONLY(P_iamu0)
IF(.true.) CALL F_iemiss%GET_DEVICE_DATA_RDONLY(P_iemiss)
IF(.true.) CALL F_its%GET_DEVICE_DATA_RDONLY(P_its)
IF(.true.) CALL F_islm%GET_DEVICE_DATA_RDONLY(P_islm)
IF(.true.) CALL F_iccnl%GET_DEVICE_DATA_RDONLY(P_iccnl)
IF(.true.) CALL F_iccno%GET_DEVICE_DATA_RDONLY(P_iccno)
IF(.true.) CALL F_igelam%GET_DEVICE_DATA_RDONLY(P_igelam)
IF(.true.) CALL F_igemu%GET_DEVICE_DATA_RDONLY(P_igemu)
IF(.true.) CALL F_iald%GET_DEVICE_DATA_RDONLY(P_iald)
IF(.true.) CALL F_ialp%GET_DEVICE_DATA_RDONLY(P_ialp)
IF(.true.) CALL F_iti%GET_DEVICE_DATA_RDONLY(P_iti)
IF(.true.) CALL F_ipr%GET_DEVICE_DATA_RDONLY(P_ipr)
IF(.true.) CALL F_iwv%GET_DEVICE_DATA_RDONLY(P_iwv)
IF(.true.) CALL F_iclc%GET_DEVICE_DATA_RDONLY(P_iclc)
IF(.true.) CALL F_ilwa%GET_DEVICE_DATA_RDONLY(P_ilwa)
IF(.true.) CALL F_iiwa%GET_DEVICE_DATA_RDONLY(P_iiwa)
IF(.true.) CALL F_iswa%GET_DEVICE_DATA_RDONLY(P_iswa)
IF(.true.) CALL F_irwa%GET_DEVICE_DATA_RDONLY(P_irwa)
IF(lrayfm) CALL F_ioz%GET_DEVICE_DATA_RDONLY(P_ioz)
IF(.true.) CALL F_iaprs%GET_DEVICE_DATA_RDONLY(P_iaprs)
IF(.true.) CALL F_ihti%GET_DEVICE_DATA_RDONLY(P_ihti)
IF(lbitidentity) CALL F_ire_liq%GET_DEVICE_DATA_RDONLY(P_ire_liq)
IF(lbitidentity) CALL F_ire_ice%GET_DEVICE_DATA_RDONLY(P_ire_ice)
IF(lbitidentity) CALL F_ioverlap%GET_DEVICE_DATA_RDONLY(P_ioverlap)

IF(.true.) CALL F_iaero%GET_DEVICE_DATA_RDWR(P_iaero)
IF(.true.) CALL F_ifrsod%GET_DEVICE_DATA_RDWR(P_ifrsod)
IF(.true.) CALL F_ifrted%GET_DEVICE_DATA_RDWR(P_ifrted)
IF(.true.) CALL F_ifrsodc%GET_DEVICE_DATA_RDWR(P_ifrsodc)
IF(.true.) CALL F_ifrtedc%GET_DEVICE_DATA_RDWR(P_ifrtedc)
IF(.true.) CALL F_iemit%GET_DEVICE_DATA_RDWR(P_iemit)
IF(.true.) CALL F_isudu%GET_DEVICE_DATA_RDWR(P_isudu)
IF(.true.) CALL F_iuvdf%GET_DEVICE_DATA_RDWR(P_iuvdf)
IF(.true.) CALL F_iparf%GET_DEVICE_DATA_RDWR(P_iparf)
IF(.true.) CALL F_iparcf%GET_DEVICE_DATA_RDWR(P_iparcf)
IF(.true.) CALL F_itincf%GET_DEVICE_DATA_RDWR(P_itincf)
IF(.true.) CALL F_ifdir%GET_DEVICE_DATA_RDWR(P_ifdir)
IF(.true.) CALL F_icdir%GET_DEVICE_DATA_RDWR(P_icdir)
IF(lapproxlwupdate) CALL F_ilwderivative%GET_DEVICE_DATA_RDWR(P_ilwderivative)
IF(lapproxswupdate) CALL F_iswdirectband%GET_DEVICE_DATA_RDWR(P_iswdirectband)
IF(lapproxswupdate) CALL F_iswdiffuseband%GET_DEVICE_DATA_RDWR(P_iswdiffuseband)
IF(.true.) CALL F_ifrso%GET_DEVICE_DATA_RDWR(P_ifrso)
IF(.true.) CALL F_iswfc%GET_DEVICE_DATA_RDWR(P_iswfc)
IF(.true.) CALL F_ifrth%GET_DEVICE_DATA_RDWR(P_ifrth)
IF(.true.) CALL F_ilwfc%GET_DEVICE_DATA_RDWR(P_ilwfc)
...
!$ACC DATA COPYIN(ISEED) IF(PRESENT(ISEED))

IF (LHOOK) CALL DR_HOOK('RADIATION_SCHEME_LAYER_OPENACC:OFFLOAD',1,ZHOOK_HANDLE_OFFLOAD)
IF (LHOOK) CALL DR_HOOK('RADIATION_SCHEME_LAYER_OPENACC:COMPUTE',0,ZHOOK_HANDLE_COMPUTE)
...
END DO
IF (LHOOK) CALL DR_HOOK('RADIATION_SCHEME_LAYER_OPENACC:COMPUTE',1,ZHOOK_HANDLE_COMPUTE)

IF (LHOOK) CALL DR_HOOK('RADIATION_SCHEME_LAYER_OPENACC:PULLBACK',0,ZHOOK_HANDLE_PULLBACK)

!$ACC WAIT(1)
!$ACC END DATA
CALL ZRGP_FIELDS%FIELD_WRAPPER%SYNC_HOST_RDWR()

IF (LHOOK) CALL DR_HOOK('RADIATION_SCHEME_LAYER_OPENACC:PULLBACK',1,ZHOOK_HANDLE_PULLBACK)

END ASSOCIATE
END ASSOCIATE

IF (LHOOK) CALL DR_HOOK('RADIATION_SCHEME_LAYER_OPENACC',1,ZHOOK_HANDLE)
