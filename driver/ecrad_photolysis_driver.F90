! ecrad_photolysis_driver.F90 - Driver for offline ECRAD radiation scheme with photolysis calculation
!
...
program ecrad_photolysis_driver
...
!#include "satur.intfb.h"
  ...
  character(len=512) :: photolysis_file_name
...
!  real(jprb) :: weight(20)
  ...
  if (command_argument_count() < 4) then
    stop 'Usage: ecrad_photolysis config.nam input_file.nc output_file.nc photolysis_output_file.nc'
  end if
  ...
  if (driver_config%iverbose >= 2) then
    write(nulout,'(a)') '------------------ OFFLINE ECRAD RADIATION SCHEME WITH PHOTOLYSIS ------------------'
  ...
  ! Get photolysis NetCDF output file name
  call get_command_argument(4, photolysis_file_name, status=istatus)
  if (istatus /= 0) then
    do_photolysis = .false.
    stop 'Failed to read name of photolysis output NetCDF file as string of length < 512'
  else
    do_photolysis = .true.
  end if
  ...
  do jrepeat = 1,driver_config%nrepeat
    ...
    if (driver_config%do_parallel) then
      ...
      if (do_photolysis) then
        do jcol = driver_config%istartcol,driver_config%iendcol
          call photolysis%calculate(jcol, single_level%cos_sza(jcol), &
               &  thermodynamics%temperature_hl(jcol,:), flux, photolysis_rate(:,:,jcol))
        end do
      end if
...
end program ecrad_photolysis_driver
