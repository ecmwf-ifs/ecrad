...
def main(latitude, input_srcfile, reference_output_srcfile, output_srcfiles, dstdir):
    ...
    dstfile = f"{dstdir}/{name_string}_{outputs_string}_vs_{reference_string}_profile_{format_latitude(latitude)}.png"
    
    print(f"Plotting output profiles to {dstfile}")
    eplt.compare_output_profile(latitude, input_srcfile, reference_output_srcfile, output_srcfiles, styles, dstfile=dstfile)
